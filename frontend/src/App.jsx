...
export default function App() {
  ...
  const fetchDemo = useCallback(async () => {
    ...
    try {
      const { data } = await axios.get(`${API}/api/demo`);
      setReport(data);
    } catch {
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg)"}}>
        ...
        <div style={{maxWidth:1200,margin:"28px auto",padding:"0 20px"}}>
          <div style={{background:"var(--red-lt)",border:"1px solid var(--red-md)",
            ...
            <div style={{marginTop:8,fontSize:12,color:"var(--text3)"}}>
              Run:&nbsp;<code style={{background:"var(--bg2)",padding:"2px 8px",
      ...
      <footer style={{textAlign:"center",padding:"24px 20px",color:"var(--text4)",
        fontSize:11,borderTop:"1px solid var(--border)",marginTop:40,background:"var(--white)",
        fontFamily:"var(--mono)",letterSpacing:"0.06em"}}>
        AutoParts AI · Supply Chain Intelligence · Python + FastAPI + React
      </footer>
    </div>
  );
}